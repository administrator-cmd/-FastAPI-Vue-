...
# 取消评论点赞
async def get_comment_like_by_user_and_comment(db, comment_id, user_id) -> Optional[CommentLike]:
...
async def delete_comment_like(db : AsyncSession, comment_id : int, current_user_id : int) -> bool:
    """取消评论点赞"""
    commentlike_obj = await get_comment_like_by_user_and_comment(db, comment_id, current_user_id)
