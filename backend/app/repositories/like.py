...
from app.repositories import get_post_by_id, get_user_by_id
...
async def create_like(db: AsyncSession, user_id: int, post_id: int) -> Like:
    ...
    # 检查是否已经点赞过
    existing_like = await get_like_by_user_and_post(db, user_id, post_id)
    if existing_like:
        raise ValueError("已经点赞过")
    db_like = Like(
        user_id = user_id,
        post_id = post_id
    )
...
async def delete_like(db: AsyncSession,user_id: int, post_id: int) -> bool:
    """异步删除点赞 只能删除自己的点赞"""
    like = await get_like_by_user_and_post(db, user_id, post_id)
    if not like:
        return False
